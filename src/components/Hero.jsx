// "use client" // Uncomment this if using Next.js App Router

import React, { useEffect, useRef, useState } from "react";
import NET from "vanta/dist/vanta.net.min"; // important: ESM/min build
import * as THREE from "three"; // pass explicit THREE to prevent mobile issues
...
const Hero = () => {
  ...
  const handleResumeDownload = () => {
    try {
      ...
      toast.error("Unable to download resume. Please try again.");
    }
  };
  ...
  try {
    ...
    vantaEffect.current = NET({
      ...
      showDots: false,
        ...
        if (canvas) {
          ...
          // Cleanup listener when we cleanup effect
          const cleanup = () => {
            canvas.removeEventListener("webglcontextlost", onContextLost);
          };
          // Store cleanup function on the ref for use in return cleanup
          vantaRef.current._cleanupContextLost = cleanup;
        }
      } catch (err) {
        console.error("Vanta init failed:", err);
        // Leave the gradient/CSS fallback visible
      }
    };
    ...
    return () => {
      ...
      try {
        // Remove context lost listener if attached
        const cleanup = vantaRef.current?._cleanupContextLost;
  ...
  return (
    <>
      <div
        ref={vantaRef}
        // Use min-h-screen and w-full to avoid iOS h-screen/w-screen bugs.
        // Provide a graceful CSS background in case WebGL is unavailable.
        className="relative min-h-screen w-full flex flex-col justify-center items-center text-white overflow-visible"
        style={{
          background:
            "radial-gradient(1200px circle at 50% 40%, rgba(255,255,255,0.06), rgba(0,0,0,1) 60%)",
      ...
      >
        ...
        >
          {/* Main Button */}
          <button
            className="text-zinc-200 hover:text-red-400 backdrop-blur-lg
            bg-gradient-to-tr from-transparent via-[rgba(121,121,121,0.16)]
            to-transparent rounded-md py-2 px-6 shadow
            hover:shadow-red-600 duration-700 border border-white/10"
          >
            My Links
          </button>
              ...
              <div className="flex space-x-4">
                ...
                <div className="relative">
                  <button
                    ...
                    hover:scale-110 hover:shadow-lg hover:shadow-red-600/30 border border-white/10"
                    aria-label="LinkedIn"
                  >
                  ...
                  {activeTooltip === "linkedin" && (
                    <div className="absolute top-full mt-2 left-1/2 transform -translate-x-1/2 bg-gray-800 text-white text-xs py-1 px-2 rounded whitespace-nowrap">
                      LinkedIn Profile
                    </div>
                  )}
                </div>
                ...
                <div className="relative">
                  <button
                    ...
                    transition-all duration-300 flex items-center justify-center
                    hover:scale-110 hover:shadow-lg hover:shadow-gray-600/30 border border-white/10"
                    aria-label="GitHub"
                  >
                    <svg className="w-6 h-6 fill-current text-white" viewBox="0 0 24 24">
        ...
        {/* Optional content spacer on small devices */}
